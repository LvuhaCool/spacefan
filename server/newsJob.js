...
const LL2_URL          = 'https://ll.thespacedevs.com/2.3.0/launches/upcoming/?limit=10&ordering=net&mode=detailed';
  ...
  db.transaction(() => {
    ...
    for (const l of list) {
      ...
      const landings = stages
        .filter(s => s.landing)
        .map(s => {
          const rawName = s.landing?.location?.name ?? '';
          const name = (rawName === 'Not Defined' || rawName === 'TBD') ? '' : rawName;
          return { name, type: s.landing?.type?.abbrev ?? '', reused: s.reused ?? false };
        });
